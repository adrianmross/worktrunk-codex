...
```toml
[commit.generation]
command = "codex exec -c model_reasoning_effort='low' -c system_prompt='' --sandbox=read-only --json - | jq -sr '[.[] | select(.item.type? == \"agent_message\")] | last.item.text'"
```

Uses your default Codex model with low reasoning effort and an empty system prompt for fast text-only output. Requires `jq` for JSON parsing. If you want a dedicated profile or model override, add `-p <profile>` or `-m <model>`. See [Codex CLI docs](https://developers.openai.com/codex/cli/).
