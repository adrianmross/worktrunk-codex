---
name: worktrunk
description: Guidance for Worktrunk (the `wt` CLI) inside Codex. Load when using or debugging `wt`, editing `.config/wt.toml` or `~/.config/worktrunk/config.toml`, creating or removing worktrees, configuring commit generation, or troubleshooting shell integration and hooks.
version: 0.44.0-codex.1
...
Help users work with Worktrunk, a CLI for managing git worktrees.

## Codex Plugin Behavior

This Codex plugin adds three things on top of the upstream Worktrunk reference set:

1. **Worktrunk skill docs** — synced reference files from `worktrunk.dev`
2. **Activity markers** — Codex hooks set `wt` branch markers:
   - `SessionStart` -> `💬`
   - `UserPromptSubmit` -> `🤖`
   - `Stop` -> `💬`
3. **Worktree guardrail** — a `PreToolUse` Bash hook blocks raw `git worktree add/remove` commands and tells Codex to use `wt`

See `reference/codex-plugin.md` for plugin-specific details and limitations.
...
For command-specific options, run `wt <command> --help`. For Codex sessions, follow these rules first:

## Codex Rules

1. **Use `wt`, not raw `git worktree`, for lifecycle actions**
   - Create a new parallel worktree: `wt switch --create <branch>`
   - Create a stacked follow-up branch from the current worktree: `wt switch --create <branch> --base=@`
   - Return to the main worktree: `wt switch ^`
   - Remove the current worktree: `wt remove @`

2. **Treat config scope correctly**
   - User preferences go in `~/.config/worktrunk/config.toml`
   - Shared repo automation goes in `.config/wt.toml`
...
2. **If none installed, recommend Codex or Claude Code**
...
grep -A 30 "## Codex Plugin" reference/codex-plugin.md
```
